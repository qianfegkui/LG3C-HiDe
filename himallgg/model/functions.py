...
def batch_graphifyt(features, lengths, speaker_tensor, wp, wf, edge_type_to_idx, device):
    node_features, edge_index, edge_type = [], [], []
    batch_size = features.size(0)
    length_sum = 0
    edge_ind = []
    ...
    for j in range(batch_size):
        ...
        for item, item_rec in zip(perms, perms_rec):
            ...
            if item[0] < item[1]:
                ...
                c = '1'
            edge_type.append(edge_type_to_idx[c])  # 直接使用c作为键
...
def batch_graphify_(features, lengths, speaker_tensor, wp, wf, edge_type_to_idx, device):
    node_features, edge_index, edge_type = [], [], []
    batch_size = features.size(0)
    length_sum = 0
    edge_ind = []
